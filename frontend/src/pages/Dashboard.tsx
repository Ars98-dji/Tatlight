...
interface Purchase { order_number: string; product_id: string; product_title: string; product_image: string | null; price: number; purchased_at: string; download_url: string | null; }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-12 p-8 bg-gradient-to-br from-[#D4AF37]/10 to-transparent border border-[#D4AF37]/30 rounded-3xl">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            <div className="relative group cursor-pointer" onClick={() => fileRef.current?.click()}>
              {user?.avatar ? (
                <img src={user.avatar} alt={user.full_name} className="w-24 h-24 rounded-full object-cover border-4 border-[#D4AF37]/30" />
              ) : (
                <div className="w-24 h-24 rounded-full bg-[#D4AF37]/10 border-4 border-[#D4AF37]/30 flex items-center justify-center">
              ...
              <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-[#D4AF37] rounded-full flex items-center justify-center border-4 border-white">
            ...
            <div className="flex flex-col items-center gap-2 px-6 py-4 bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-2xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#D4AF37] font-bold text-lg">{loyalty?.tier || user?.loyalty_tier || 'Bronze'}</span>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-[#D4AF37]">{loyalty?.total_points || user?.loyalty_points || 0}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
          {stats.map((stat, index) => (
            <div key={index} className="p-6 bg-gradient-to-br from-[#D4AF37]/5 to-transparent border border-[#D4AF37]/20 rounded-2xl hover:border-[#D4AF37]/40 transition-all">
        ...
        {loyalty && loyalty.next_tier && (
          <div className="mb-8 p-4 bg-[#D4AF37]/5 rounded-2xl border border-[#D4AF37]/20">
            <div className="flex items-center justify-between mb-2">
              <span className="text-gray-700 text-sm">Progression vers <span className="text-[#D4AF37] font-semibold">{loyalty.next_tier}</span></span>
              <span className="text-[#D4AF37] text-sm font-semibold">{loyalty.progress_to_next_tier}%</span>
            </div>
            <div className="w-full bg-gray-50 rounded-full h-2 overflow-hidden">
              <div className="bg-[#D4AF37] h-full rounded-full transition-all" style={{ width: `${loyalty.progress_to_next_tier}%` }} />
            </div>
          </div>
        )}
        ...
        {rewards.length > 0 && (
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-3xl font-bold text-gray-900">Récompenses <span className="text-[#D4AF37]">Fidélité</span></h2>
              <button onClick={() => setShowRewards(!showRewards)} className="text-[#D4AF37] font-semibold hover:underline text-sm">
            ...
            {showRewards && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {rewards.map((reward) => {
                  ...
                  return (
                    <div key={reward.id} className={`p-6 rounded-2xl border transition-all ${canAfford ? 'bg-gradient-to-br from-[#D4AF37]/10 to-transparent border-[#D4AF37]/30' : 'bg-gray-50 border-gray-200 opacity-75'}`}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold text-[#D4AF37]">{reward.points_required} pts</span>
                        </div>
                        <button
                          onClick={() => handleRedeem(reward.id)}
                          disabled={!canAfford || redeeming === reward.id}
                          className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${canAfford ? 'bg-[#D4AF37] text-[#0D1B2A] hover:glow-gold' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
                        >
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-3xl font-bold text-gray-900">Mes <span className="text-[#D4AF37]">Achats</span></h2>
          </div>
          {purchases.length === 0 ? (
            <div className="p-12 text-center bg-gradient-to-br from-[#D4AF37]/5 to-transparent border border-[#D4AF37]/20 rounded-2xl">
              <ShoppingBag className="w-16 h-16 text-[#D4AF37]/50 mx-auto mb-4" />
              ...
              <Link to="/" className="inline-flex items-center gap-2 px-6 py-3 bg-[#D4AF37] text-[#0D1B2A] rounded-full font-semibold hover:glow-gold transition-all">
            ...
            <div className="space-y-4">
              {purchases.map((purchase, idx) => (
                <div key={idx} className="p-6 bg-gradient-to-br from-[#D4AF37]/5 to-transparent border border-[#D4AF37]/20 rounded-2xl hover:border-[#D4AF37]/40 transition-all group">
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="w-full md:w-48 flex-shrink-0">
                      <div className="relative aspect-[4/3] rounded-xl overflow-hidden bg-[#D4AF37]/10">
                        {purchase.product_image ? (
                          <img src={purchase.product_image} alt={purchase.product_title} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center"><FileText className="w-8 h-8 text-[#D4AF37]/50" /></div>
                        )}
                      </div>
                    </div>
                    <div className="flex-grow flex flex-col justify-between">
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-[#D4AF37] transition-colors">{purchase.product_title}</h3>
                        <div className="flex items-center gap-4 text-gray-500 text-sm mb-4">
                          ...
                          <div className="text-[#D4AF37] font-semibold">{purchase.price}€</div>
                        </div>
                      </div>
                      <div className="flex gap-3">
                        {purchase.download_url && (
                          <a href={purchase.download_url} className="flex items-center gap-2 px-6 py-2 bg-[#D4AF37] text-[#0D1B2A] rounded-full font-semibold hover:glow-gold transition-all">
                            <Download className="w-4 h-4" /> Télécharger
                          </a>
                        )}
                        <Link to={`/produit/${purchase.product_id}`} className="flex items-center gap-2 px-6 py-2 bg-[#D4AF37]/10 border border-[#D4AF37]/30 text-[#D4AF37] rounded-full font-semibold hover:bg-[#D4AF37]/20 transition-all">
                          Voir Détails
                        </Link>
                      </div>
                    </div>
